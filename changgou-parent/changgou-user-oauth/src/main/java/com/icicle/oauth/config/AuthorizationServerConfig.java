...
class AuthorizationServerConfig extends AuthorizationServerConfigurerAdapter {
    ...
    public void configure(ClientDetailsServiceConfigurer clients) throws Exception {
        ...
        //目前属于硬编码 实际需要从数据库加载
        clients.jdbc(dataSource).clients(clientDetails());  //从数据库加载
        //硬编码
//        clients.inMemory()
...
//                .scopes("app");                         //客户端范围，名称自定义，必填
    ...
    public JwtAccessTokenConverter jwtAccessTokenConverter(CustomUserAuthenticationConverter customUserAuthenticationConverter) {
        ...
        KeyPair keyPair = new KeyStoreKeyFactory(
                keyProperties.getKeyStore().getLocation(),                          //证书路径 icicle.jks
                keyProperties.getKeyStore().getSecret().toCharArray())              //证书秘钥 changgouapp
                .getKeyPair(
                        keyProperties.getKeyStore().getAlias(),                     //证书别名 icicle
                        keyProperties.getKeyStore().getPassword().toCharArray());   //证书密码 icicle
